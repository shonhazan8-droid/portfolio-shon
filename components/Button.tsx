...
export default function Button({ href, variant = "primary", children, className = "", style, target, rel }: Props) {
  ...
  return <Link href={href} target={target} rel={rel} className={`${base} ${styles[variant]} ${className}`} style={style}>{children}</Link>;
}
